...
// specifically for wikis - redlinks tooltip adds this message
		, de = mw && mw.config && mw.config.get('wgUserLanguage') == 'de-formal '
		, pageDoesntExistMessage = de ? '(page does not exist)' : '(Seite existiert nicht)'
		, expandLegend = de ? 'Show Legend' : 'Legende ausklappen'
		, collapseLegend = de ? 'Hide Legend' : 'Legende einklappen'
		;
    ...
    function drawMarker(context, areas) { // this is where the magic is done.
  	...
  	function init() {
  		mw.util.addCSS('li.' + myClassName + '{white-space:nowrap;font-size:90%;}\n' + //css for li element
  					'li.' + liHighlightClass + '{background-color:yellow;font-size:90%;}\n' + //css for highlighted li element.
  					'.rtl li.' + myClassName + '{float: right; margin-left: 1em;}\n' +
  					'.ltr li.' + myClassName + '{float: left; margin-right: 1em;}');
  ...
  // (other scripts adding elements, window resize etc.), canvas and imagese remain aligned.
  			...
  			var ol = $('<ol style="font-size:90%;">', {'class': myClassName})
